...
//如果多项式排序单链表PolySLinkedList的泛型参数是TermX，则Polynomial表示一元多项式
//如果多项式排序单链表PolySLinkedList的泛型参数是TermXY，则Polynomial表示二元多项式
public class Polynomial                                    //多项式类
{
    private PolySLinkedList<TermX> list;                   //多项式排序单链表，TermX表示一元多项式的一项

    public Polynomial()                                    //默认构造方法
    {
        this.list = new PolySLinkedList<TermX>();          //创建空单链表，执行排序单链表默认构造方法
    }
 ...
 //           System.out.println("start="+start+", end="+end+", "+polystr.substring(start,end));
    ...
    public Polynomial(Polynomial poly)                     //深度拷贝构造方法，复制单链表的所有结点和元素对象
    {
        this();                                            //创建空单链表，只有头结点
        ...
        while (p!=null)
        {
            rear.next = new Node<TermX>(new TermX(p.data), null);//复制结点，复制元素对象
            rear = rear.next; 
            p = p.next;
        }
    }
    ...
    public void add(Polynomial poly)                       //多项式相加，this＋=poly
    {
        this.list.add(poly.list);
    }
    public Polynomial plus(Polynomial poly)                //加法＋，C=this＋poly
    {
    	Polynomial polyc=new Polynomial(this);             //深度拷贝，复制所有结点及元素对象
        polyc.add(poly);
        return polyc;                                      //返回对象引用
    }
