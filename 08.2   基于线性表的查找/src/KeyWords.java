...
public class KeyWords  
{
    ...
    public static boolean isKeyword(String str)            //判断str是否为Java关键字
    {
        ...
        int pos=BinarySearchArray.binarySearch(index, item);               //折半查找索引表，获得索引项位置
        int begin=index[pos].start;                                //获得主表查找范围的下界
        int end=index[pos+1].start-1;                           //获得主表查找范围的上界
        return BinarySearchArray.binarySearch(keywords,begin,end,str)>=0;   //折半查找主表指定范围
    }
